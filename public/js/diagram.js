...
{		
  ...
  */	
  ...
  {	
    ...
    domain_y1 = y1.domain([d3.min(data.map(function(d) { return d.v; })), d3.max(data.map(function(d) { return d.v; }))]);
    domain_y2 = y2.domain([d3.min(data.map(function(d) { return d.a; })), d3.max(data.map(function(d) { return d.a; }))]); 
    var minimum = 0;
    ...
    y_1.domain([0, d3.max(data.map(function(d) { return d.x; }))]);
    y_2.domain([0, d3.max(data.map(function(d) { return d.y; }))]);
...
function reload(file)
{
  ...
  let PatientHeading = document.getElementById("PatientHeading");
  renderVis(file);
  if(file=="CSVs/data6.csv" || file=="CSVs/data5.csv"){
    PatientHeading.innerHTML= " Representing 'patient2' data";
  }
}
